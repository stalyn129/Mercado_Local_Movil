  ...
  class _ProductoDetallePageState extends State<ProductoDetallePage> with TickerProviderStateMixin {
    ...
    bool loading = true;
    bool favorito = false;
    int? idFavorito; // Guardamos el ID del favorito para poder eliminarlo
    ...
    Future<void> cargarProducto() async {
      try {
        ...
        if (user != null && token != null) {
          final idConsumidor = user["idConsumidor"];

          final favRes = await dio.get(
            "$API_URL/favoritos/listar/$idConsumidor",
            options: Options(headers: {
              "Authorization": "Bearer $token",
            }),
          );

          // Buscar si este producto está en favoritos
          for (var f in favRes.data) {
            if (f["idProducto"] == widget.idProducto) {
              esFavorito = true;
              favId = f["idFavorito"];
              break;
            }
        ...
        // 🔸 Versión temporal (mientras configuras AuthService):
        setState(() {
          producto = res.data;
          loading = false;
        });
    ...
    Future<void> toggleFavorito() async {
      try {
        ...
        if (!favorito) {
          ...
          // ❌ QUITAR DE FAVORITOS
          if (idFavorito == null) return;

          await dio.delete(
            "$API_URL/favoritos/eliminar/$idFavorito",
            options: Options(
              headers: {"Authorization": "Bearer $token"},
            ),
          );
